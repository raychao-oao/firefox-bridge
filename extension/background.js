...
// tabId -> { documentId, hostname, tools: Map<toolName, toolMeta> }. See
// docs/superpowers/specs/2026-08-20-firefox-bridge-webmcp-shim-design.md
// ("Registry data model and lifecycle") for why documentId (Firefox's own
// native per-document identifier, never a shim-invented UUID) is the key
// correctness primitive here -- it's what lets onCommitted cleanup (Task 6)
// tell a genuinely-stale registration apart from one that just won a race
// against the new document's own document_start registration. `hostname`
// is the whitelist-authorized hostname stored AT REGISTRATION TIME --
// webmcp_list_tools/webmcp_call_tool (Task 5) re-check it against the
// CURRENT webmcpWhitelist on every call, so removing a hostname from the
// whitelist revokes AI-callability immediately even though this entry
// itself is untouched.
const webmcpToolRegistry = new Map();

// requestId -> { resolve, timer, tabId, documentId } for in-flight
// webmcp_call_tool calls. Populated by handleWebmcpCallTool (Task 5),
// resolved/rejected here by the webmcp-tool-result/webmcp-tool-error
// handlers below. tabId/documentId let those handlers verify a result
// truly came from the document a call was dispatched to. Deliberately
// NOT keyed/bound by sessionId -- an earlier revision added one so
// onSessionEnded could reject only its own session's pending calls, but
// a scoped review judged that disproportionate for this single-user
// feature's realistic call volume (see Task 6). Abandoned calls simply
// self-expire within WEBMCP_CALL_TIMEOUT_MS.
const webmcpPendingCalls = new Map();
...
browser.runtime.onMessage.addListener((msg, sender) => {
  ...
  if (msg.type === 'webmcp-tool-register') {
    // Real enforcement point for the whole feature (see Task 4's header
    // note and spec's "Whitelist mechanism" § "Enforcement point"): the
    // relay (webmcp-relay.js) has no whitelist awareness, so a
    // non-whitelisted page could otherwise self-forge this exact message
    // via window.postMessage without any shim ever being injected.
    //
    // sender.url (the SENDING DOCUMENT's own URL) is used here, NOT
    // sender.tab.url -- a tab's .url is tab-level state that can lag or
    // lead the document that actually sent this particular message
    // during navigation; it is not proof of which document sent it.
    // sender.frameId === 0 additionally enforces top-level-frame-only,
    // matching the no-iframe-support scope decision at this layer too.
    if (sender.frameId !== 0) return;
    const hostname = normalizeWebmcpHostname(sender.url);
    if (!isWebmcpHostnameAllowed(hostname, webmcpWhitelist)) return; // reject silently -- no error posted back, so an unauthorized page gets no signal distinguishing "rejected" from "nobody's listening"

    // A whitelisted page can still send a malformed forged envelope (not
    // just an unwhitelisted one) -- validate shape before touching the
    // registry, rather than dereferencing msg.tool.name unconditionally
    // and throwing inside this handler.
    if (!msg.tool || typeof msg.tool.name !== 'string') return;

    // sender.documentId is Firefox's own native per-document identifier
    // (Firefox 153+, see Global Constraints) -- never generated by this
    // extension. A new document (different documentId) always starts a
    // fresh tool set; it never inherits a prior document's tools for the
    // same tabId. hostname is stored on the entry -- see the Map comment
    // above for why (live whitelist re-check happens in Task 5, not here).
    const documentId = sender.documentId;
    let entry = webmcpToolRegistry.get(tabId);
    if (!entry || entry.documentId !== documentId) {
      entry = { documentId, hostname, tools: new Map() };
      webmcpToolRegistry.set(tabId, entry);
    }
    entry.tools.set(msg.tool.name, msg.tool);
    return;
  }

  if (msg.type === 'webmcp-tool-result' || msg.type === 'webmcp-tool-error') {
    // Binding validation, NOT a whitelist re-check -- see this task's
    // header note on why re-checking the whitelist here would create a
    // "timeout black hole" for calls already authorized and dispatched.
    // A result/error must come from the exact document its pending call
    // was dispatched to. requestId is an unguessable crypto.randomUUID()
    // (Task 5), so this tabId/documentId check is belt-and-suspenders,
    // not the primary defense.
    if (sender.frameId !== 0) return;
    const pending = webmcpPendingCalls.get(msg.requestId);
    if (!pending) return; // already timed out or unknown -- see WEBMCP_CALL_TIMEOUT_MS handling in Task 5
    if (sender.tab.id !== pending.tabId || sender.documentId !== pending.documentId) return;

    clearTimeout(pending.timer);
    webmcpPendingCalls.delete(msg.requestId);
    if (msg.type === 'webmcp-tool-result') {
      pending.resolve({ ok: true, result: msg.result });
    } else {
      pending.resolve({ ok: false, error: `tool_execution_error: ${msg.error}` });
    }
    return;
  }
});
