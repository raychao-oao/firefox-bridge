...
export function registerTools(server, bridgeClient) {
  ...
  server.registerTool(
    ...
    {
      description:
        "Type text into an element in a leased tab, identified by a CSS selector. This ALWAYS overwrites the element's existing value with `text` — there is no append/insert-at-cursor mode, whatever was there before is replaced. Pass `frameId` to target a specific frame (get it from `list_frames` or a `list_elements` entry) — defaults to the top frame (0). If you omit `frameId` AND the element is not found in the top frame, this tool automatically falls back to searching up to 20 other accessible frames for the first one where the selector resolves — you do NOT need to pass `frameId` just because an element came from an iframe. The response includes `frameSearchIncomplete: true` only alongside an ultimately unsuccessful result (`element_not_found`/`stale_selector`) when that omitted-`frameId` search could NOT be exhaustive (a frame was policy-blocked, the 20-frame cap was hit, or frame enumeration failed) — never alongside a successful type. Optionally pass `expectedDomEpoch` (from a prior `list_elements` call, same `frameId`) to guard against acting on a stale selector — if the page has since changed (navigation, bfcache restoration), this returns `stale_selector` and does NOT type anything.",
